...
- **Log parser:** large-parse performance (parse-scoped lookup caches, set-based `CreateTopAliases`, batched `mysqli_multi_query` for queued `UPDATE`s, `INSERT … ON DUPLICATE KEY UPDATE` for player/time stats).
...
- **Install:** `UltraStats_ValidateTablePrefix` before prefixed DDL; `gen_gameversion` and `database_installedversion` via `DB_ExecBound` after schema batch; upgrade runner accepts single-statement `db_update_*.txt` files (statement count check uses `< 1`).
