...
- Tests live under [`e2e/`](e2e/) ([`e2e/tests/00-install-wizard.spec.ts`](e2e/tests/00-install-wizard.spec.ts) runs first so the DB is populated before [`frontend-admin-smoke.spec.ts`](e2e/tests/frontend-admin-smoke.spec.ts)). To run Playwright against an already-up stack from the host: `cd e2e && npm ci && set PLAYWRIGHT_BASE_URL=http://127.0.0.1:8092&& npx playwright test` (Unix: `export PLAYWRIGHT_BASE_URL=...`).
- CI: [.github/workflows/install-e2e.yml](.github/workflows/install-e2e.yml) (on **push to `main`**, **pull requests**, and **manual** `workflow_dispatch`). Each run uploads artifact **`install-e2e-reports`** (adds **`install-e2e-run.log`**): open **`install-e2e-report/index.html`** for a step-by-step screenshot gallery, or **`playwright-report/index.html`** for the full Playwright HTML report (includes the same shots as attachments). Artifacts are kept **14 days**. On **failure** (push, `workflow_dispatch`, or **same-repo** PRs only — not fork PRs), the workflow **opens a GitHub issue** with a log tail and links to the run and artifact; **PNGs are not inlined** in the issue (download the artifact to view them — GitHub’s default token cannot create gists for inline images).
