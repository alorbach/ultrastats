...
def fetch_generated_notes(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ultrastats-release-workflow",
        },
    )
    try:
...
def main() -> int:
    ...
    if not tag_name or not tag_name.startswith("v"):
        print("GITHUB_REF_NAME must be a v-prefixed tag (e.g. v0.3.15)", file=sys.stderr)
        return 1
    raw_ver = tag_name[1:]
    # Use base X.Y.Z for ChangeLog (ignore -rc / +build suffixes on the tag)
    ...
    cl_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "ChangeLog"
    )
